...
from vit_for_track_classification import ViTForTrackClassification
from tqdm import tqdm

logger = logging.getLogger(__name__)


def get_track_dataset_dict(dir: Path) -> datasets.Dataset:
    class_names, class_to_idx = find_classes(dir)

    samples = {"images": [], "label": []}
    for class_name, class_id in class_to_idx.items():
        class_dir = dir / class_name
        for track in tqdm(
            class_dir.iterdir(), desc=f"Searchging for {class_name} tracks"
        ):
            if not track.is_dir():
                continue
            track_path = class_dir / track
            images = [str(i) for i in track_path.glob("*.jpg")]
            samples["images"].append(images)
            samples["label"].append(class_id)
...
def main():
    ...
    ds_dict = {}
    ds_list = [
        get_track_dataset_dict(Path(train_dir)) for train_dir in data_args.train_dir
    ]
    ...
    if data_args.validation_dir is not None and len(data_args.validation_dir) > 0:
        ds_list = [
            get_track_dataset_dict(Path(validation_dir))
    ...
    labels = np.asarray(dataset["train"]["label"])
    ...
    model = ViTForTrackClassification.from_pretrained(
    ...
    model.loss_fct = torch.nn.CrossEntropyLoss(
        label_smoothing=0.2, weight=class_weights
    )
    ...
    def train_transforms(example_batch):
        ...
        decoder = datasets.Image(decode=True)

        train_images = []
        for bi in range(len(example_batch["images"])):
            images = example_batch["images"][bi]
            image_indices = np.random.randint(
                0, len(example_batch["images"]) + 1, size=10
            )
            train_images.append(
                torch.stack(
                    [
                        _train_transforms(
                            image=pil_to_numpy(decoder.decode_example(images[i]))
                        )["image"]
        ...
        example_batch["pixel_values"] = torch.stack(train_images)
