...
def main():
    ...
    ### only run the feature extraction with gt
    ...
    try:
        known_individuals = load_semi_gt_ids(date=args.date, camera_id=cam_id)
    except Exception as e:
        logger.exception("Failed to load semi-GT: %s", e)
        return 1
    # # known_individuals = load_semi_gt_ids(date=args.date, camera_id=cam_id)
    # if len(known_individuals) == 0:
    #     ## speed up - only process the data with semi-gt
    #     logger.warning("No known individuals found in semi-GT for camera %s on date %s. Please ensure semi-GT is available. Exiting.", cam_id, args.date)
    #     return 1
    
    # if len(known_individuals) == 1:
    #     logger.warning("Multiple known individuals found in semi-GT for camera %s on date %s. This script is designed for single-camera processing with one known individual. Please check the semi-GT data. Exiting.", cam_id, args.date)
    #     return 1
    
    if len(known_individuals) <= 1:
        logger.warning("No Group found for camera %s on date %s", cam_id, args.date)
        return 0
