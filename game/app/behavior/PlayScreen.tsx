...
import { BehaviorQuestion } from "../../lib/types";
...
export default function PlayScreen({
  ...
  const handleAnswer = useCallback(
    (behavior: string) => {
      if (disabled) return;
      setDisabled(true);
      setFeedback(behavior === question.behavior ? "correct" : "wrong");
      setTimeout(() => {
        setFeedback(null);
        setDisabled(false);
        onAnswer(behavior);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (idx >= 0 && idx < BEHAVIORS.length) handleAnswer(BEHAVIORS[idx].name);
    };
  ...
  return (
    <div className="w-full max-w-xl">
      ...
      <div className={`rounded-3xl border bg-white/[0.03] backdrop-blur-2xl shadow-2xl p-8 transition-all duration-500 ${feedbackBorder}`}>
        <h2 className="text-center text-2xl font-extralight tracking-wide text-white/90 mb-2">
          What is this elephant doing?
        </h2>
        <p className="text-center text-sm text-white/40 mb-6">{question.elephant_name}</p>
        ...
        <div className="grid grid-cols-1 gap-3">
          {BEHAVIORS.map((behavior, idx) => (
            <button
              key={behavior.id}
              onClick={() => handleAnswer(behavior.name)}
              disabled={disabled}
              ...
              style={{ borderColor: `${behavior.color}30` }}
            >
              <span className="text-xs text-white/40 w-5">{idx + 1}</span>
              <span className="w-3 h-3 rounded-full" style={{ backgroundColor: behavior.color }} />
              <span style={{ color: behavior.color }}>{BEHAVIOR_LABELS[behavior.name] || behavior.name}</span>
            </button>
          ))}
        </div>
