...
import { BehaviorQuestion, BehaviorAnswer } from "../../lib/types";
import { loadBehaviorQuestions } from "../../lib/data";
...
export default function BehaviorPage() {
  ...
  const [questions, setQuestions] = useState<BehaviorQuestion[]>([]);
  ...
  const handleStart = async (count: number) => {
    ...
    setError(null);
    try {
      const allQuestions = await loadBehaviorQuestions();
      setQuestions(allQuestions.slice(0, count));
      setAnswers([]);
      setCurrentIndex(0);
      setPhase("play");
    } catch (e: any) {
      setError(e?.message || "Failed to load behavior data. Run generate_game_data.py first.");
    } finally {
      setLoading(false);
    }
  };
