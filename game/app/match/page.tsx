...
import { MatchPair, MatchAnswer } from "../../lib/types";
import { loadMatchPairs } from "../../lib/data";
...
export default function MatchPage() {
  ...
  const handleStart = async (count: number) => {
    ...
    setError(null);
    try {
      const allPairs = await loadMatchPairs();
      setPairs(allPairs.slice(0, count));
      setAnswers([]);
      setCurrentIndex(0);
      setPhase("play");
    } catch (e: any) {
  ...
  const handleAnswer = (userAnswer: boolean) => {
    ...
    const newAnswers = [...answers, { pair, userAnswer }];
  ...
  return (
    <div className="fixed inset-0 overflow-auto bg-[#050510]">
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-12">
        ...
        {phase === "results" && (
          <ResultsScreen answers={answers} onPlayAgain={handlePlayAgain} />
        )}
      </div>
    </div>
  );
