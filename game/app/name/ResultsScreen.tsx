...
import { NameAnswer } from "../../lib/types";
...
export default function ResultsScreen({
  ...
  const correct = answers.filter((a) => a.userAnswer === a.question.elephant_name).length;
  ...
  const message =
    pct >= 90 ? "Amazing! You truly know these elephants!"
    : pct >= 70 ? "Great job! Impressive elephant knowledge."
    : pct >= 50 ? "Not bad! Keep practicing."
    : "Keep trying — you'll learn them!";
    ...
    <div className="w-full max-w-2xl">
      ...
      <div className="rounded-3xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-2xl shadow-2xl p-6">
        ...
        <div className="space-y-3">
          {answers.map((a, i) => {
            const isCorrect = a.userAnswer === a.question.elephant_name;
            const correctElephant = getElephantByName(a.question.elephant_name);
            const answeredElephant = getElephantByName(a.userAnswer);
            return (
              <div key={i} className={`grid grid-cols-[auto_1fr_auto] items-center gap-5 rounded-2xl border p-4 transition-colors ${
                ...
                <div className="min-w-0">
                  <div className="text-sm font-normal text-white/70 truncate">
                    Correct: <span style={{ color: correctElephant?.color }}>{a.question.elephant_name}</span>
                  </div>
                  {!isCorrect && (
                    <div className="text-xs text-white/50 mt-1">
                      You said: <span style={{ color: answeredElephant?.color }}>{a.userAnswer}</span>
                    </div>
                  )}
                </div>
