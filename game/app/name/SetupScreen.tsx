              ...
              <button
                ...
                className={`rounded-2xl border px-4 py-4 text-xl font-light transition-all duration-300 ${
                  !useCustom && selected === n
                    ? "border-amber-400/40 bg-amber-400/[0.08] text-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.12)]"
        ...
        <div className="mb-8">
          ...
          {useCustom && (
            <input type="number" min={1} max={50} value={custom} onChange={(e) => setCustom(e.target.value)} placeholder="1–50"
              className="mt-2 w-full rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-base text-white/90 placeholder-white/30 outline-none focus:border-amber-400/30 transition-colors font-light"
            />
          )}
        </div>

        <button onClick={() => onStart(count)} disabled={loading}
          className="w-full rounded-2xl border border-amber-400/25 bg-amber-400/[0.06] px-6 py-4 text-lg font-light tracking-wide text-amber-300 transition-all duration-300 hover:bg-amber-400/[0.12] hover:shadow-[0_0_40px_rgba(245,158,11,0.15)] hover:scale-[1.02] active:scale-[0.97] active:brightness-125 disabled:opacity-30 disabled:pointer-events-none"
        >
          {loading ? (
