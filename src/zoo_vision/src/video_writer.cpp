...
void VideoWriter::write(const uint8_t *imgRgb, int stride, int width, int height) {
  cv::Mat3b im = cv::Mat3b(height, width, reinterpret_cast<cv::Vec3b *>(const_cast<uint8_t *>(imgRgb)), stride);
  writer_ << im;
}

void VideoWriter::close() { writer_.release(); }
